 ...
 */
 ...
 const globalReactJQueryInitializer = (function ($) {
    ...
    /*--------------------------------------------------------
     / 1. Revolution Slider 01
     /----------------------------------------------------------*/
     var revapi = window.jQuery('#rev_slider_1').show().revolution({
        delay: 7000,
    ...
    /*--------------------------------------------------------
     / 2. Revolution Slider 02
     /----------------------------------------------------------*/
    var revapi2 = window.jQuery('#rev_slider_2').show().revolution({
        delay: 7000,
        responsiveLevels: [1200, 1140, 778, 480],
        gridwidth: [1140, 920, 700, 380],
        sliderLayout: 'fullscreen',
   ...
   window.$('.search-toggles').on('click', function (e) {
    ...
    window.$('.scroll-down a').on('click', function () {
        window.$('html, body').animate({scrollTop: window.$(this.hash).offset().top - 68}, 1000);
        return false;
    });
    ...
    window.$(window).on("load resize", function (e) {
        if (window.$(window).width() < 991) {
            ...
            window.$('.mobile-menu ul li.menu-item-has-children > span.submenu-toggler').on('click', function () {
                var $this = window.$(this);

                if (window.$(this).hasClass('active-span')) {
                    window.$('i', $this).removeClass('twi-caret-up').addClass('twi-caret-down');
                } else {
                    window.$('i', $this).addClass('twi-caret-up').removeClass('twi-caret-down');
                }
        ...
        window.$(".wc_payment_methods li").each(function(){
            window.$('input[type="radio"]', this).on('click', function(e){
                ...
                }else
                {
                    ...
                    window.$('div.' + ids).slideDown('fast').addClass('visibales');
                }
            });
        });
    }
    ...
    {
        window.$(".menu_popup ul li.menu-item-has-children > a").on('click', function (e) {
            e.preventDefault();
    ...
    if (slider_range.length > 0) {
        slider_range.slider({
            ...
            slide: function (event, ui) {
                window.$("#slider-range span.ui-slider-handle").eq(0).html("<span>" +"$" + ui.values[ 0 ] + "</span>");
                window.$("#slider-range span.ui-slider-handle").eq(1).html("<span>" +"$" + ui.values[ 1 ] + "</span>");
            }
        });
        window.$("#slider-range span.ui-slider-handle").eq(0).html("<span>" +"$" + window.$("#slider-range").slider("values", 0) + "</span>");
        window.$("#slider-range span.ui-slider-handle").eq(1).html("<span>" +"$" + window.$("#slider-range").slider("values", 1) + "</span>");
    }
